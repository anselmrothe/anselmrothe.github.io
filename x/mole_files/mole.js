...
debug_navigation_buttons = function() {
    ...
    navigation.appendChild(document.createTextNode('v3.0'));
...
canvas_setup = function() {
    ...
    box1.drawImage(mole_img, 0, 0, 100, 100);
...
draw_box2 = function() {
    ...
    box2.font = fontsize(box2_text_size);
...
hole_shrink_update = function() {
    ...
    if (percent < recent_hole.relative_size) {
...
compute_score = function() {
    ...
    // percent of worst possible distance
    max_diff = Math.sqrt(200*200 + 200*200) // corner to corner
    percent = (max_diff - diff) / max_diff;
    ...
    //     0 |   25
    //     0 |    0
    // score = Math.max(0, 2*Math.round((percent * 100) - 50));
...
run_task_predict_patterns = function() {
    ...
    next(function() {
        // first pattern is pattern_id==0 ("1 line"), rest randomized
        pattern_ids_randomized = [0];
        randomized_rest = _.shuffle(_.range(1, patterns.length));
        pattern_ids_randomized = pattern_ids_randomized.concat(randomized_rest);
        records.push({
            time: Date.now(),
...
main = function() {
    ...
    if (DBUG_MOUSE) {
        ...
        next(function() {
            ...
            alert('TODO: save data... use JSON.stringify(records)');
        })
        timeline_run(); 
    }
}
...
run_task_generate_patterns = function() {
    run_task_generate_pattern = function(easy_or_difficult_local) {
        ...
        next(function() {
            instruction_top = [
            'Please generate a pattern',
            'that is: ' + easy_or_difficult,
            ''
            ];
        ...
        for (var i = 0; i < steps_per_pattern; i++) {
            next(function() {
                ...
                //TODO: button.text = 'Reset last click';
                //TODO: save last elements in timeline and reinsert them?
                //TODO: i don't think a reset button only for the last click is useful
            });
            wait_for_click_box1();
            next(function() {
                step++;
    ...
    if (Math.random() > .5) {
        for (var j = 0; j < n_trials_generate_easy; j++) {
            run_task_generate_pattern('EASY');
        }
        for (var j = 0; j < n_trials_generate_hard; j++) {
            run_task_generate_pattern('DIFFICULT');
        }
    } else {
        for (var j = 0; j < n_trials_generate_hard; j++) {
            run_task_generate_pattern('DIFFICULT');
...
next_intro = function() {
    ...
    next(function() {
        recent_hole.visible = true;
        instructions = ['A small gray dot shows', 'where the mole has been.', '', 
...
// use this function if difficulty in chrome or safari
// from http://diveinto.html5doctor.com/canvas.html
...
//  // canvas (document.getElementById('box1')), not context
//  // this returns a PNG as text
//  // (google data uri to png)
...
//  box2.strokeStyle = 'black';
//  box2.strokeRect(x, y, width, height);
//  box2.textBaseline = 'middle';
//  box2.fillStyle = 'black';
//  while (true) {
//      // adjust font size
//      box2.font = fontsize(text_size);
//      t = box2.measureText(button.text);
//      if (t.width < width) {
//          break;
//      }
//      text_size--;
//  }
//  box2.fillText(button.text, x + width/2 - t.width/2, y + height/2 + 1)
// }


//// not so good timeline variant
...
//      var outer_timeline_before = timeline.slice(0, timeline_i); // [event1, event2] (event3 is 'inner_timeline' and gets replaced with the inner timeline)
//      var outer_timeline_after  = timeline.slice(timeline_i+1); // [event4, event5]
//      timeline = []; // []
//      build_inner_timeline(); // timeline = [eventA, eventB]
//      timeline = outer_timeline_before.concat(timeline.concat(outer_timeline_after)); // [event1, event2, eventA, eventB, event4, event5]
//      // set timeline_i one back because the current event (inner_timeline) is now replaced
//      timeline_i -= 1;
//  });
// }
...
//  any_events_left = timeline_i < timeline.length;
//  if (any_events_left) {
//      // event = timeline.shift();
//      event = timeline[timeline_i];
//      event();
//      draw_box1();
//      draw_box2();
//      if (!timeline_paused) {
